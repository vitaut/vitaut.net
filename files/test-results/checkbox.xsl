...
<xsl:template match="/">
    ...
    <head>
        ...
        <link rel="stylesheet" type="text/css" href="/usr/share/checkbox/report/styles.css" />
        <script type="text/javascript" src="/usr/share/checkbox/report/checkbox.js">
        </script>
    </head>
    <body>
        <!-- Quick and dirty preload to have disclosure triangle images cached -->
        <img src="/usr/share/checkbox/report/images/open.png" style="display: none;" /><img src="/usr/share/checkbox/report/images/closed.png" style="display: none;" />
        <div id="container">
...
<xsl:template match="hardware/udev">
    <div onClick="showHide('udev');">
        <h3 id="udev"><img class="disclosureimg" src="/usr/share/checkbox/report/images/closed.png" />Devices detected in the system (udev)</h3>
    </div>
...
<xsl:template match="hardware/dmi">
    <span onClick="showHide('dmi');"><h3 id="dmi"><img class="disclosureimg" src="/usr/share/checkbox/report/images/closed.png" />Desktop Management Interface information</h3></span>
...
<xsl:template match="hardware/sysfs-attributes">
    <span onClick="showHide('sysfs-attributes');"><h3 id="sysfs-attributes"><img class="disclosureimg" src="/usr/share/checkbox/report/images/closed.png" />sysfs-attributes</h3></span>
...
<xsl:template match="hardware/processors">
    <span onClick="showHide('processors');"><h3 id="processors"><img class="disclosureimg" src="/usr/share/checkbox/report/images/closed.png" />Processors</h3></span>
...
<xsl:template match="software/packages">
    <span onClick="showHide('packages');"><h3 id="packages"><img class="disclosureimg" src="/usr/share/checkbox/report/images/closed.png" />Packages Installed</h3></span>
            ...
            <tr>
                <xsl:choose>
                    <xsl:when test="normalize-space(answer) = 'fail'">
                        <td><img class='resultimg' src='/usr/share/checkbox/report/images/fail.png' /></td>
                        <td class="label"><xsl:value-of select="@name" /></td>
                        <td style='background-color: #f00'>FAILED</td>
                        <td><xsl:value-of select="comment" /></td>
                    </xsl:when>
                    <xsl:when test="normalize-space(answer) = 'pass'">
                        <td><img class='resultimg' src='/usr/share/checkbox/report/images/pass.png' /></td>
                        <td class="label"><xsl:value-of select="@name" /></td>
                    ...
                    <xsl:when test="normalize-space(answer) = 'untested'">
                        <td><img class='resultimg' src='/usr/share/checkbox/report/images/skip.png' /></td>
                        <td class="label"><xsl:value-of select="@name" /></td>
...
<xsl:template match="context">
	...
	<span onClick="showHide('{generate-id(.)}');"><h3 id="{generate-id(.)}"><img class="disclosureimg" src="/usr/share/checkbox/report/images/closed.png" />
